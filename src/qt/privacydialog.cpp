                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zPEPS ought to be enough for anybody." - Bill Gates, 2017
    ui->zPEPSpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzPEPSSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzPEPS->setEnabled(false);
        ui->pushButtonMintzPEPS->setToolTip(tr("zPEPS is currently disabled due to maintenance."));

        ui->pushButtonSpendzPEPS->setEnabled(false);
        ui->pushButtonSpendzPEPS->setToolTip(tr("zPEPS is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zPEPSpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzPEPS_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zPEPS is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zPEPS...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zPEPS in ") +
...
void PrivacyDialog::on_pushButtonSpendzPEPS_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zPEPS is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zPEPS
        sendzPEPS();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zPEPS
    sendzPEPS();
}

void PrivacyDialog::on_pushButtonZPepsControl_clicked()
{
    ZPepsControlDialog* zPepsControl = new ZPepsControlDialog(this);
    zPepsControl->setModel(walletModel);
    zPepsControl->exec();
}

void PrivacyDialog::setZPepsControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzPepsSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzPEPS()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Peps Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zPEPSpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zPEPS is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " PEPS </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zPEPS</b>";
    ...
    // use mints from zPeps selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZPepsControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZPepsControlDialog::GetSelectedMints();
    }
    ...
    // Clear zpeps selector in case it was used
    ZPepsControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zPeps Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Peps, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zPeps Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zPEPS </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zPEPS "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zPEPS "));
    ui->labelzPEPSAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzPEPSSyncStatus->setVisible(fShow);
}
