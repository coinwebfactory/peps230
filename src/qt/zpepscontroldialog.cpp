...
// Copyright (c) 2017-2018 The Peps developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zpepscontroldialog.h"
#include "ui_zpepscontroldialog.h"
...
std::list<std::string> ZPepsControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZPepsControlDialog::listMints;

ZPepsControlDialog::ZPepsControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZPepsControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZPepsControlDialog::~ZPepsControlDialog()
{
    delete ui;
}

void ZPepsControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZPepsControlDialog::updateList()
{
...
void ZPepsControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZPepsControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZPeps_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZPepsControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZPepsControlDialog::GetSelectedMints()
{
...
void ZPepsControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
